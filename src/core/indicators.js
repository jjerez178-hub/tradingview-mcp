...
export async function setInputs({ entity_id, inputs: inputsRaw, _deps } = {}) {
  ...
  return { success: true, entity_id, updated_inputs: result.updated_inputs, verified: true, as_of: new Date().toISOString() };
}
...
export async function toggleVisibility({ entity_id, visible, _deps } = {}) {
  ...
  if (result && result.error) throw new Error(result.error);
  return { success: true, entity_id, visible: result.visible, verified: true, as_of: new Date().toISOString() };
}
